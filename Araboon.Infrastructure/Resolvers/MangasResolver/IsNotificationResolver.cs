...
{
    public class IsNotificationResolver : IValueResolver<Manga, GetMangaByIDResponse, Boolean>
    {
        private readonly IHttpContextAccessor httpContextAccessor;
        private readonly AraboonDbContext _context;
        public IsNotificationResolver(IHttpContextAccessor httpContextAccessor, AraboonDbContext _context)
        {
        ...
        public Boolean Resolve(Manga source, GetMangaByIDResponse destination, Boolean destMember, ResolutionContext context)
        {
            try
            {
                ...
                if (String.IsNullOrEmpty(userId))
                    return false;
                return _context.Notifications.Any(f => f.UserID.ToString().Equals(userId) && f.MangaID.Equals(source.MangaID));
            }
            catch
            {
                return false;
