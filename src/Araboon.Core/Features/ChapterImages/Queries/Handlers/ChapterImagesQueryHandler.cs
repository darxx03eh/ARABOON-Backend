...
namespace Araboon.Core.Features.ChapterImages.Queries.Handlers
{
        ...
        , IRequestHandler<GetChapterImagesQuery, ApiResponse>
    {
        ...
        {
            var (result, images, mangaName, chaptersCount) = await chapterPhotoService.GetChapterImagesAsync(request.MangaId, request.ChapterNo, request.Language);
            return result switch
            {
                ...
                "ThisLanguageNotExist" => NotFound(stringLocalizer[SharedTranslationKeys.ThisLanguageNotExist]),
